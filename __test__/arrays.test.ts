import { describe, expect, test, it } from "@jest/globals";
import "../src/arrays";

describe("Arrays Extensions test cases", () => {
  it("should return true when given an empty array", () => {
    expect([].isEmpty()).toBe(true);
  });
  it("should return false when given a non-empty array", () => {
    const arr = [1, 2, 3];
    expect(arr.isEmpty()).toBe(false);
  });
  it("should return true when given an array containing only null values", () => {
    const arr = [null, null, null];
    expect(arr.isEmpty()).toBe(false);
  });
  it("should return false when given an array containing special characters", () => {
    const arr = [1, 2, 3, null];
    expect(arr.isEmpty()).toBe(false);
  });

  // Tests that the function returns true when called on an array with one element
  it("should return true when called on an array with one element", () => {
    const arr = [1];
    expect(arr.isNotEmpty()).toBe(true);
  });

  // Tests that the function returns true for an empty array
  it("should return true when array is empty", () => {
    const arr: any = [];
    expect(arr.isEmptyOrNull()).toBe(true);
  });

  // Generated by CodiumAI

  describe("groupBy", () => {
    // Tests that groupBy function returns an object with keys grouped by the given key
    it("should return an object with keys grouped by the given key when the array is not empty", () => {
      const arr = [
        { id: 1, name: "John" },
        { id: 2, name: "Jane" },
        { id: 3, name: "John" },
        { id: 4, name: "Jane" },
      ];
      const result = arr.groupBy("name");
      expect(result).toEqual({
        John: [
          { id: 1, name: "John" },
          { id: 3, name: "John" },
        ],
        Jane: [
          { id: 2, name: "Jane" },
          { id: 4, name: "Jane" },
        ],
      });
    });

    // Tests that groupBy function returns an empty object when the array is empty
    it("should return an empty object when the array is empty", () => {
      const arr: any = [];
      const result = arr.groupBy("name");
      expect(result).toEqual({});
    });
  });

  // Tests that the function returns an empty array when given an empty array.
  it("should return an empty array when given an empty array", () => {
    const arr: any = [];
    const result = arr.removeDuplicates();
    expect(result).toEqual([]);
  });

  // Tests that an array with unique objects returns the same array
  it("should return the same array when the input array has unique objects", () => {
    const input = [
      { id: 1, name: "John" },
      { id: 2, name: "Jane" },
      { id: 3, name: "Bob" },
    ];
    const expected = [
      { id: 1, name: "John" },
      { id: 2, name: "Jane" },
      { id: 3, name: "Bob" },
    ];
    const result = input.removeDuplicates();
    expect(result).toEqual(expected);
  });

  // Tests that an array with one object returns the same array
  it("should return the same array when the input array has one object", () => {
    const input = [{ id: 1, name: "John" }];
    const expected = [{ id: 1, name: "John" }];
    const result = input.removeDuplicates();
    expect(result).toEqual(expected);
  });

  // Tests that an empty array returns an empty array
  it("should return an empty array when the input array is empty", () => {
    const input: any = [];
    const expected: any = [];
    const result = input.removeDuplicates();
    expect(result).toEqual(expected);
  });

  // Tests that an array with multiple objects with the same id and name returns an array with unique objects
  it("should return an array with unique objects when the input array has multiple objects with the same id and name", () => {
    const input = [
      { id: 1, name: "John" },
      { id: 2, name: "Jane" },
      { id: 1, name: "John" },
      { id: 3, name: "Bob" },
      { id: 2, name: "Jane" },
    ];
    const expected = [
      { id: 1, name: "John" },
      { id: 2, name: "Jane" },
      { id: 3, name: "Bob" },
    ];
    const result = input.removeDuplicates();
    expect(result).toEqual(expected);
  });

  // Tests that sortBy sorts an array of objects by a given key in ascending order
  it("should sort an array of objects in ascending order when given a key", () => {
    const arr = [
      { name: "John", age: 25 },
      { name: "Alice", age: 30 },
      { name: "Bob", age: 20 },
    ];
    const sortedArr = arr.sortBy("name");
    expect(sortedArr).toEqual([
      { name: "Alice", age: 30 },
      { name: "Bob", age: 20 },
      { name: "John", age: 25 },
    ]);
  });

  // Tests that sortBy sorts an array of objects by a given key in descending order
  it("should sort an array of objects in descending order when given a key", () => {
    const arr = [
      { name: "John", age: 25 },
      { name: "Alice", age: 30 },
      { name: "Bob", age: 20 },
    ];
    const sortedArr = arr.sortBy("name").reverse();
    expect(sortedArr).toEqual([
      { name: "John", age: 25 },
      { name: "Bob", age: 20 },
      { name: "Alice", age: 30 },
    ]);
  });

  // Tests that firstOrDefault method
  it("should return the first element of the array when the array is not empty", () => {
    const arr = [1, 2, 3];
    const result = arr.firstOrDefault();
    expect(result).toEqual(1);
  });

  // Tests that firstOrDefault method returns null when the array is empty
  it("should return null when the array is empty", () => {
    const arr: any = [];
    const result = arr.firstOrDefault();
    expect(result).toEqual(null);
  });

  // Tests that firstOrDefault method returns the default value when the array is empty
  it("should return the default value when the array is empty", () => {
    const arr: any = [];
    const result = arr.firstOrElse(1);
    expect(result).toEqual(1);
  });

  // Tests that lastOrDefault method
  it("should return the last element of the array when the array is not empty", () => {
    const arr = [1, 2, 3];
    const result = arr.lastOrDefault();
    expect(result).toEqual(3);
  });

  // Tests that lastOrDefault method returns null when the array is empty
  it("should return null when the array is empty", () => {
    const arr: any = [];
    const result = arr.lastOrDefault();
    expect(result).toEqual(null);
  });

  // Tests that lastOrDefault method returns the default value when the array is empty
  it("should return the default value when the array is empty", () => {
    const arr: any = [];
    const result = arr.lastOrDefault();
    expect(result).toEqual(null);
  });

  // Tests that firstOrElse method returns the default value when the array is empty
  it("should return the default value when the array is empty", () => {
    const arr: any = [];
    const result = arr.firstOrElse(1);
    expect(result).toEqual(1);
  });

  // Tests that firstOrElse method returns the first element of the array when the array is not empty
  it("should return the first element of the array when the array is not empty", () => {
    const arr = [1, 2, 3];
    const result = arr.firstOrElse(() => 5);
    expect(result).toEqual(1);
  });

  // test skip function
  it("should return an array with the first element removed", () => {
    const arr = [5, 6, 6];
    const result = arr.skip(1);
    expect(result).toEqual([6, 6]);
  });

  // test take function
  it("should return an array with the first element removed", () => {
    const arr = [5, 6, 6];
    const result = arr.take(1);
    expect(result).toEqual([5]);
  });
});
